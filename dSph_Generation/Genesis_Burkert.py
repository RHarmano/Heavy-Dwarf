...
if __name__ == '__main__':
    ...
    for galaxy in range(num_galaxy):
        ...
        mass = []
        for r in r_eval:
            mass.append(mass_from_density(r, rho_args))
        ...
        dSph_keys = ['nu_dist', 'N', 'L', 'r_half', 'DM_dist',
                    'r_core', 'rho_central', 'beta', 'D', 'DM_mass']
        ...
        save_path = "./dSph_data/{}_stars_test".format(str(N))
        if not os.path.isdir(save_path):
            os.mkdir(save_path)
        
        # write dSph metadata to top of the file
        currentDT = currentDT.strftime("%Y_%m_%d_%H_%M_%S_%f")
        save_location = os.getcwd() + '\\dSph_data\\{}_stars_test\\dSph_'.format(N) + currentDT + '.csv'
...
if __name__ == "__main__":
    ...
    #plt.tight_layout()
    plt.savefig('./Final_Figs/3Dplummer_speed_rh{}.pdf'.format(0.4*r_core), format='pdf', transparent = True, dpi=1200, bbox_inches = 'tight', pad_inches = 0)
    ...
    plt.savefig('./Final_Figs/2Dplummer_speed_rh{}.pdf'.format(0.4*r_core), format='pdf', transparent = True, dpi=1200, bbox_inches = 'tight', pad_inches = 0)
