...
def U_fun(r,beta,nu_args, rho_dist, rho_args):
   ...
   # M = M_fun(r, r_core, rho_central)
    ...
    integrand = lambda r:r**((2*beta)-2)*nu_fun(r,L,r_half)*Mass_fun(r,burkert,rho_args)
...
if __name__ == "__main__":
    ...
    for i, (dSph) in enumerate(dSph_list):
        ...
        ax.set_title("$r_h = {}$".format(0.4*r_core_list[i]), size = 30)
        ...
        plt.tight_layout()
        plt.savefig('plummer_position3D_rh{}.pdf'.format(0.4*r_core_list[i]), format='pdf', dpi=1200, transparent = True, bbox_inches = 'tight', pad_inches = 0)
    ...
    for i, (dSph) in enumerate(dSph_project_list):
        ...
        plt.tight_layout()
        plt.savefig('plummer_position2D_rh{}.pdf'.format(0.4*r_core_list[i]), format='pdf', dpi=1200, transparent = True, bbox_inches = 'tight', pad_inches = 0)
