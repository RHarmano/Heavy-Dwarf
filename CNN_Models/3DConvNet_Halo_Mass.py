...
class ConvModel(Model):
    ...
    def __init__(self):
        ...
        self.conv1 = Conv3D(32, (3,3,3), activation='relu', input_shape=galaxy_shape, kernel_regularizer=regularizers.l2(0.001), padding='valid')  # compute first convolution
        self.batch1 = BatchNormalization()
        self.maxpool1 = MaxPool3D((2,2,2))  # conduct max pooling of data
        self.conv2 = Conv3D(32, (3,3,3), activation='relu', kernel_regularizer=regularizers.l2(0.001), padding='valid')  # compute second convolution
        self.batch2 = BatchNormalization()
        self.maxpool2 = MaxPool3D((2,2,2))  # conduct max pooling of data
        self.flatten = Flatten()  # flatten the data
        self.dense1 = Dense(32, activation='relu', kernel_regularizer=regularizers.l2(0.001))  # compute the first dense neural network layer
...
save_path = "./dSph_data/Mass_Regression_Results_Drop_Reg"
...
save_location = os.getcwd() + '\\dSph_data\\Mass_Regression_Results_Drop_Reg\\Mass_Regress_{}_'.format(EPOCHS) + currentDT + '.csv'
